...
更准确地说，这套制度是**隋朝创制、唐朝完善**：中书省草拟政令，门下省审议封驳，尚书省执行。三省是分工制衡链路，不是三套并行执行队列。
...
三省六部制不是一个 metaphor，它是一套经过 1400 年实践检验的分权制衡系统。并行主要发生在尚书省统筹下的诸部执行阶段，而不是“三省并行决策”。
...
这就是这套制度能长期稳定运行的原因之一：**不受制约的权力必然会出错**，而制度化审议能显著降低决策失误。
...
隋唐制度早就给出了答案：**分权制衡**。规划的不审核，审核的不执行，执行的不规划。每一个环节都有人盯着，每一个决策都要经过审议。
